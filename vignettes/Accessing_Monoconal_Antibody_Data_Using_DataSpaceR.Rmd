...
One can also accesss the unsummarized data from the mAb grid by calling `con$mabGrid`.
...
You can retrive values from the grid by `mab_mixture`, `donor_species`, `isotype`, `hxb2_location`, `tier`, `clade`, `virus`, and `studies`, or any variables found in the `mabGrid` field in the connection object.

There are do options of doing so: using the `retrieveMabGrid` method, or using `data.table` operations.

```{r}
# using `retrieveMabGridValue`
...
# using `data.table` operations
# retrive first 10 available viruses using mabGrid
con$mabGrid[,virus]

# retrive available clades for 1H9 mAb mixture using mabGrid
con$mabGrid[mab_mixture == "1H9", clade]
```

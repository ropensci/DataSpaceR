---
title: "Accessing Monoclonal Antibodies using DataSpaceR"
author: "Ju Yeong Kim"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Accessing Monoclonal Antibodies}
...
2. Select the mAb mixtures by filtering the mAb grid using any columns found in `con$mabGridBase`
3. Use `con$getMab()` to retreive the mAb data
...
This table is designed to mimick the mAb grid found in the app.

One can also accesss the unsummarized data from the mAb grid by calling `con$mabGridBase`.

## Filter the mAb grid

You can filter rows in the grid by specifying the values to keep in the columns found in the field `con$mabGridBase`: `mab_mixture`, `donor_species`, `isotype`, `hxb2_location`, `tiers`, `clades`, `viruses`, and `studies`. `filterMabGrid` takes the column and the values and filters the underlying tables (private fields), and when you call the `mabGridSummary` or (which is actually an [active binding](https://cloud.r-project.org/web/packages/R6/vignettes/Introduction.html#active-bindings)), it returns the filtered grid with updated `n_` columns and `geometric_mean_curve_ic50`.
...
You can retrive values from the grid by `mab_mixture`, `donor_species`, `isotype`, `hxb2_location`, `tier`, `clade`, `virus`, and `studies`, or any variables found in the `mabGridBase` field in the connection object.

```{r}
# retrieve available viruses in the filtered grid
con$retrieveMabGridValue(using = "virus")

# retrive available clades for 1H9 mAb mixture in the filtered grid
con$retrieveMabGridValue(using = "clade", mab_mixture = "1H9")

# retrive first 10 available viruses using mabGridBase 
con$mabGridBase[,virus]

# retrive available clades for 1H9 mAb mixture using mabGridBase
con$mabGridBase[mab_mixture == "1H9", clade]
```
...
There are several metadata fields that can be expored in the mAb object.

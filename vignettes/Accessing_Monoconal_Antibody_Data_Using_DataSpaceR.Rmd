...
One can also access's the unsummarized data from the mAb grid by calling `con$mabGrid`.
...
You can retrieve values from the grid by `mab_mixture`, `donor_species`, `isotype`, `hxb2_location`, `tier`, `clade`, `virus`, and `studies`, or any variables found in the `mabGrid` field in the connection object.
...
# retrieve first 10 available viruses using mabGrid
con$mabGrid[,virus]

# retrieve available clades for 1H9 mAb mixture using mabGrid

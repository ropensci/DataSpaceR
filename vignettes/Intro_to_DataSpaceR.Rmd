...
at the command prompt, you should see it there. If it's not there, create one now. Make sure you have a valid login and password. If you don't have one, go to [DataSpace](https://dataspace.cavd.org) now and set yourself up with an account.
...
To fetch data from a saved group, create a connection at the project level with a group ID. For example, we can connect to `mice` group which has group ID 208 by `getStudy`.

...
`con$availableStudies` shows the available studies in the CAVD DataSpace.
...
| --- | --- | --- |
| `availableStudies` | Field | The table of available studies. |
| `availableGroups` | Field | The table of available groups. |
| `config` | Field | Stores configuration of the connection object such as URL, path and username. |
| `getStudy` | Method | Create a `DataSpaceStudy` object by study. |
| `getGroup` | Method | Create a `DataSpaceStudy` object by group. |
| `refresh` | Method | Refresh the connection object to update available studies and groups. |
| `initialize` | Method | Initialize a `DataSpaceConnection` object. |
| `print` | Method | Print `DataSpaceConnection` object |

### `DataSpaceStudy`

| Name | Type | Description |
| --- | --- | --- |
| `study` | Field | The study name. |
| `group` | Field | The group name. |
| `availableDatasets` | Field | The table of datasets available in the study object. |
| `treatmentArm` | Field | The table of treatment arm information for the connectedstudy. Not available for all study connection. |
| `config` | Field | Stores configuration of the connection object such as URL, path and username. |
| `cache` | Field | Stores the data to avoid downloading the same tables multiple times. |
| `studyInfo` | Field | Stores the information about the study. |
| `getDataset` | Method | Get a dataset from the connection. |
| `getDatasetDescription` | Method | Get variable information. |
| `clearCache` | Method | Clear `cache`. Remove downloaded datasets. |
| `refresh` | Method | Refresh the study object to update available datasets and treatment info. |
| `initialize` | Method | Initialize `DataSpaceStudy` class. |
| `print` | Method | Print `DataSpaceStudy` class. |

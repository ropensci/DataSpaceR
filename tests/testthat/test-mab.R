...
test_that("test mab object", {
  ...
  cklist <- list(
    ...
    "studies"             = mab$studies,
    "assays"              = mab$assays,
    "variableDefinitions" = mab$variableDefinitions
  )
  ...
  expect_equal(names(cklist[["studies"]]), studnames)
  expect_equal(names(cklist[["variableDefinitions"]]), varinames)
...
test_that("test mab object results", {
  ...
  expect_true(all(mab$mabMetadata$mabMetadata$mab_mix_name_std %in% c("CH27")))
